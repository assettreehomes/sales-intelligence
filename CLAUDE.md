...
- Re-analyze (`POST /tickets/:id/analyze`) responds 202 immediately; analysis fires async to avoid request timeout
...
GET  /admin/queue/status           Queue stats (waiting, processing, failed counts + stuck tickets)
POST /admin/queue/reset            Drain/clear waiting jobs in the Vertex queue
...
VERTEX_PROJECT, VERTEX_LOCATION (us-central1), VERTEX_MODEL (gemini-2.5-pro), VERTEX_MODEL_PRESALES (gemini-2.5-flash), VERTEX_MAX_RPM (default 3)
...
- `istrainingcall` is auto-set to `true` when `overall_score >= 8.0` (promotes call to training library)
...
- **`vertexQueue.js`** — in-memory job queue with sliding 60s RPM limiter (`VERTEX_MAX_RPM`, default 3) and `MAX_CONCURRENT=3`; single Cloud Run instance keeps queue coherent
- **`autoRetry.js`** — background service (20min interval) retries `analysis_failed` tickets; blacklist approach (skips `permanent_failed`); re-downloads audio from TeleCMI before retrying (fixes GCS-deleted-on-failure bug); 3 tickets/cycle, 20s stagger, 10 attempt cap per session
